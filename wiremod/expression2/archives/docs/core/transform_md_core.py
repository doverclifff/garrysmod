...
def main() -> None:
    output_filename = "expression2_core_language_reference.txt"
    globbed = glob("*.md", root_dir=curdir)
    print(f"File name: {output_filename}")
    print("File extension: .txt")
    print("MIME type: text/plain")
    print("Creation date: 01 May 2026")
    print("\n----")
    print(
        "\nExpression 2 Core Language Reference - adapted from the Expression 2 GitHub wiki"
    )
    print(
        "\nThis plain-text document is an adaptation of a sub-set of documents\n\
from the official Expression 2 Language Reference into a single\n\
monolithic plain-text document. These documents were selected\n\
because they describe the core features of the language."
    )
    print(
